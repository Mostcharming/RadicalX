import React, { Component } from 'react';

import '@fontsource/space-grotesk';
import '../static/style.css';

class Internships extends Component {
  render() {
    return (
      <div className="">
        <div className="internWrap">
          <div className="internWrap-header">
            <span className="internWrap-text">Internships</span>
            <a href="#">
              <div className="internWrap-subhead">
                <div className="internWrap-subhead-vct">
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M12.4917 10H13.3334"
                      stroke="white"
                      stroke-width="1.5"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                    <path
                      d="M6.66663 10H9.84163"
                      stroke="white"
                      stroke-width="1.5"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                    <path
                      d="M10 13.3332V6.6665"
                      stroke="white"
                      stroke-width="1.5"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                    <path
                      d="M1.66663 10.8665V12.4998C1.66663 16.6665 3.33329 18.3332 7.49996 18.3332H12.5C16.6666 18.3332 18.3333 16.6665 18.3333 12.4998V7.49984C18.3333 3.33317 16.6666 1.6665 12.5 1.6665H7.49996C3.33329 1.6665 1.66663 3.33317 1.66663 7.49984"
                      stroke="white"
                      stroke-width="1.5"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                  </svg>
                </div>
                <span className="internWrap-subhead-text">
                  Create New Internship
                </span>
              </div>
            </a>
          </div>
          <div className="internWrap-main">
            <div className="internWrap-main-top">
              <div className="internWrap-main-top-1">
                <div className="internWrap-main-top-1-1">
                  <span className="internWrap-main-top-1-1-text">
                    Internship Insights
                  </span>
                  <span className="internWrap-main-top-1-1-text-2">
                    In the eighteenth century the German philosopher Immanuel
                    Kant developed a theory of knowledge in which knowledge
                    about space can be both a priori and synthetic.
                  </span>
                </div>
              </div>
              <div className="internWrap-main-top-2">
                <div className="iwmt2-header">
                  <div className="iwmt2-header-2">
                    <div className="iwmt2-headermini">
                      <div className="iwmt2-headermini1">
                        <span className="iwmt2-headermini-text1">
                          This week
                        </span>
                      </div>
                      <div className="iwmt2-headermini2">
                        <span className="iwmt2-headermini-text2">
                          This month
                        </span>
                      </div>
                    </div>
                    <div className="iwmt2-h2">
                      <div className="iwmt2-h2-vct">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M6.66663 4.2915C6.63648 4.2915 6.6052 4.27922 6.57955 4.25358C6.55391 4.22793 6.54163 4.19665 6.54163 4.1665V1.6665C6.54163 1.63636 6.55391 1.60508 6.57955 1.57943C6.6052 1.55379 6.63648 1.5415 6.66663 1.5415C6.69677 1.5415 6.72805 1.55379 6.7537 1.57943C6.77934 1.60508 6.79163 1.63636 6.79163 1.6665V4.1665C6.79163 4.19665 6.77934 4.22793 6.7537 4.25358C6.72805 4.27922 6.69677 4.2915 6.66663 4.2915Z"
                            fill="#793EF5"
                            stroke="#793EF5"
                          />
                          <path
                            d="M13.3334 4.2915C13.3032 4.2915 13.2719 4.27922 13.2463 4.25358C13.2207 4.22793 13.2084 4.19665 13.2084 4.1665V1.6665C13.2084 1.63636 13.2207 1.60508 13.2463 1.57943C13.2719 1.55379 13.3032 1.5415 13.3334 1.5415C13.3635 1.5415 13.3948 1.55379 13.4204 1.57943C13.4461 1.60508 13.4584 1.63636 13.4584 1.6665V4.1665C13.4584 4.19665 13.4461 4.22793 13.4204 4.25358C13.3948 4.27922 13.3635 4.2915 13.3334 4.2915Z"
                            fill="#793EF5"
                            stroke="#793EF5"
                          />
                          <path
                            d="M17.0833 7.70019H2.91663C2.88648 7.70019 2.8552 7.68791 2.82955 7.66227C2.80391 7.63662 2.79163 7.60534 2.79163 7.5752C2.79163 7.54505 2.80391 7.51377 2.82955 7.48812C2.8552 7.46248 2.88648 7.4502 2.91663 7.4502H17.0833C17.1134 7.4502 17.1447 7.46248 17.1704 7.48812C17.196 7.51377 17.2083 7.54505 17.2083 7.5752C17.2083 7.60534 17.196 7.63662 17.1704 7.66227C17.1447 7.68791 17.1134 7.70019 17.0833 7.70019Z"
                            fill="#793EF5"
                            stroke="#793EF5"
                          />
                          <path
                            d="M13.3333 18.4582H6.66667C5.24074 18.4582 4.18404 18.0497 3.48376 17.3494C2.78348 16.6491 2.375 15.5924 2.375 14.1665V7.08317C2.375 5.65725 2.78348 4.60055 3.48376 3.90027C4.18404 3.19999 5.24074 2.7915 6.66667 2.7915H13.3333C14.7593 2.7915 15.816 3.19999 16.5162 3.90027C17.2165 4.60055 17.625 5.65725 17.625 7.08317V14.1665C17.625 15.5924 17.2165 16.6491 16.5162 17.3494C15.816 18.0497 14.7593 18.4582 13.3333 18.4582ZM6.66667 3.0415C5.40151 3.0415 4.36464 3.34892 3.64853 4.06503C2.93242 4.78115 2.625 5.81802 2.625 7.08317V14.1665C2.625 15.4317 2.93242 16.4685 3.64853 17.1846C4.36464 17.9008 5.40151 18.2082 6.66667 18.2082H13.3333C14.5985 18.2082 15.6354 17.9008 16.3515 17.1846C17.0676 16.4685 17.375 15.4317 17.375 14.1665V7.08317C17.375 5.81801 17.0676 4.78115 16.3515 4.06503C15.6354 3.34892 14.5985 3.0415 13.3333 3.0415H6.66667Z"
                            fill="#793EF5"
                            stroke="#793EF5"
                          />
                          <path
                            d="M7.08333 12.0832C6.975 12.0832 6.86666 12.0582 6.76666 12.0165C6.66666 11.9749 6.57501 11.9165 6.49167 11.8415C6.41667 11.7582 6.35832 11.6665 6.31666 11.5665C6.27499 11.4665 6.25 11.3582 6.25 11.2498C6.25 11.0332 6.34167 10.8165 6.49167 10.6582C6.57501 10.5832 6.66666 10.5248 6.76666 10.4832C6.91666 10.4165 7.08334 10.3998 7.25001 10.4332C7.30001 10.4415 7.35 10.4582 7.4 10.4832C7.45 10.4998 7.5 10.5249 7.55 10.5582C7.59166 10.5915 7.63333 10.6249 7.67499 10.6582C7.70833 10.6999 7.74999 10.7415 7.77499 10.7832C7.80832 10.8332 7.83334 10.8832 7.85001 10.9332C7.87501 10.9832 7.89168 11.0332 7.90001 11.0832C7.90834 11.1415 7.91667 11.1915 7.91667 11.2498C7.91667 11.4665 7.82499 11.6832 7.67499 11.8415C7.51666 11.9915 7.3 12.0832 7.08333 12.0832Z"
                            fill="#793EF5"
                          />
                          <path
                            d="M9.99996 12.0832C9.78329 12.0832 9.56663 11.9916 9.4083 11.8416C9.37497 11.7999 9.34164 11.7582 9.3083 11.7166C9.27497 11.6666 9.24995 11.6166 9.23328 11.5666C9.20828 11.5166 9.19162 11.4666 9.18328 11.4166C9.17495 11.3582 9.16663 11.3082 9.16663 11.2499C9.16663 11.1416 9.19161 11.0332 9.23328 10.9332C9.27495 10.8332 9.3333 10.7416 9.4083 10.6582C9.64163 10.4249 10.0166 10.3499 10.3166 10.4832C10.425 10.5249 10.5083 10.5832 10.5916 10.6582C10.7416 10.8166 10.8333 11.0332 10.8333 11.2499C10.8333 11.3082 10.825 11.3582 10.8166 11.4166C10.8083 11.4666 10.7916 11.5166 10.7666 11.5666C10.75 11.6166 10.7249 11.6666 10.6916 11.7166C10.6583 11.7582 10.625 11.7999 10.5916 11.8416C10.5083 11.9166 10.425 11.9749 10.3166 12.0166C10.2166 12.0582 10.1083 12.0832 9.99996 12.0832Z"
                            fill="#793EF5"
                          />
                          <path
                            d="M7.08333 15.0002C6.975 15.0002 6.86666 14.9752 6.76666 14.9336C6.66666 14.8919 6.57501 14.8335 6.49167 14.7585C6.41667 14.6752 6.35832 14.5919 6.31666 14.4836C6.27499 14.3836 6.25 14.2752 6.25 14.1669C6.25 13.9502 6.34167 13.7336 6.49167 13.5752C6.57501 13.5002 6.66666 13.4419 6.76666 13.4002C7.075 13.2669 7.44166 13.3419 7.67499 13.5752C7.70833 13.6169 7.74999 13.6586 7.77499 13.7002C7.80832 13.7502 7.83334 13.8002 7.85001 13.8502C7.87501 13.9002 7.89168 13.9502 7.90001 14.0086C7.90834 14.0586 7.91667 14.1169 7.91667 14.1669C7.91667 14.3836 7.82499 14.6002 7.67499 14.7585C7.51666 14.9085 7.3 15.0002 7.08333 15.0002Z"
                            fill="#793EF5"
                          />
                        </svg>
                      </div>
                      <span className="iwmt2-h2-text">Select dates</span>
                    </div>
                  </div>
                </div>
                <div className="iwmt2-body">
                  <span className="btxt-1">Total Enrollments</span>
                  <span className="btxt-2">100,000</span>
                  <span className="btxt-3">Completion</span>
                  <span className="btxt-4">65,000</span>
                  <span className="btxt-5">Candiates Hired</span>
                  <span className="btxt-6">150</span>
                  <span className="btxt-7">Interview set</span>
                  <span className="btxt-8">300</span>
                  <span className="btxt-9">Reached Out</span>
                  <span className="btxt-10">375</span>

                  <div className="graph-vct">
                    <svg
                      width="789"
                      height="94"
                      viewBox="0 0 789 94"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M260.419 66.0002C226.608 68.6568 192.767 82.0001 87.2138 12.4644C70.3824 1.37624 40.9777 -2.35485 28.9354 3.00017C15.0752 9.16363 10.496 12.6741 0 23.4994V94.0002H789V86.5783C785.387 86.987 766.029 84.5592 730.722 83.5783C686.587 82.3522 676.717 83.5783 644.323 83.5783C611.929 83.5783 577.851 84.7263 546.105 83.5002C514.36 82.2741 488.029 82.7917 445.443 78.5002C411.374 75.0671 306.102 66.8176 260.419 66.0002Z"
                        fill="#665FEF"
                        fill-opacity="0.2"
                      />
                    </svg>
                  </div>
                  <div className="line1"></div>
                  <div className="line2"></div>
                  <div className="line3"></div>
                  <div className="bvctact-pallete">
                    <div className="bvctact-p-text">
                      <span className="bvctact-p-text-1">
                        Qualified Candiates
                      </span>
                      <span className="bvctact-p-text-2">850</span>
                      <div className="bvctact-p-text-vct">
                        <svg
                          width="133"
                          height="111"
                          viewBox="0 0 133 111"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M-152.304 7.85376C-189.099 -15.1994 -237.795 19.5029 -257.543 39.7357V110.244H536.816V99.8209C533.178 100.23 517.015 100.802 481.468 99.8209C437.034 98.5947 408.19 83.8799 377.009 72.2308C345.827 60.5816 300.613 50.1586 268.651 48.9324C236.69 47.7062 219.54 39.7357 176.665 35.4439C166.624 34.4387 150.497 84.9295 132.13 90.9998C78.3347 82.9998 33.3791 83.621 0.086731 82.9998C-1.38577 82.9723 -2.81258 28.7241 -4.1904 28.6996C-38.2307 31.3565 -115.51 30.9069 -152.304 7.85376Z"
                            fill="#793EF5"
                          />
                        </svg>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="iwm-buttom">
              <div className="iwmb-1">
                <div className="iwmb1-head">
                  <span className="iwmb1h-text">Internship Title</span>
                  <div className="iwmb1h-vct">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 20 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g opacity="0.5">
                        <path
                          d="M1.66663 10.8248V12.4998C1.66663 16.6665 3.33329 18.3332 7.49996 18.3332H12.5C16.6666 18.3332 18.3333 16.6665 18.3333 12.4998V7.49984C18.3333 3.33317 16.6666 1.6665 12.5 1.6665H7.49996C3.33329 1.6665 1.66663 3.33317 1.66663 7.49984"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.05835 8.8667L10 11.8L12.9417 8.8667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </g>
                    </svg>
                  </div>
                </div>
                <div className="iwmb1-body">
                  <div className="iwmb1-b-box">
                    <span className="iwmb1-b-btxt1">Product Design GVI</span>
                    <span className="iwmb1-b-btxt2">
                      Amet minim mollit non deserunt est sit aliqua dolor do
                      amet sint.{' '}
                    </span>
                  </div>
                </div>
                <div className="iwmb1-body-2">
                  <div className="iwmb1-b-box">
                    <span className="iwmb1-b-btxt1">Product Design GVI</span>
                    <span className="iwmb1-b-btxt2">
                      Amet minim mollit non deserunt est sit aliqua dolor do
                      amet sint.{' '}
                    </span>
                  </div>
                </div>
                <div className="iwmb1-body-3">
                  <div className="iwmb1-b-box">
                    <span className="iwmb1-b-btxt1">Product Design GVI</span>
                    <span className="iwmb1-b-btxt2">
                      Amet minim mollit non deserunt est sit aliqua dolor do
                      amet sint.{' '}
                    </span>
                  </div>
                </div>
                <div className="iwmb1-body-4">
                  <div className="iwmb1-b-box">
                    <span className="iwmb1-b-btxt1">Product Design GVI</span>
                    <span className="iwmb1-b-btxt2">
                      Amet minim mollit non deserunt est sit aliqua dolor do
                      amet sint.{' '}
                    </span>
                  </div>
                </div>
                <div className="iwmb1-body-5">
                  <div className="iwmb1-b-box">
                    <span className="iwmb1-b-btxt1">Product Design GVI</span>
                    <span className="iwmb1-b-btxt2">
                      Amet minim mollit non deserunt est sit aliqua dolor do
                      amet sint.{' '}
                    </span>
                  </div>
                </div>
              </div>
              <div className="iwmb-2">
                <div className="iwmb2-head">
                  <span className="iwmb2h-text">Completion Period</span>
                  <div className="iwmb2h-vct">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 20 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g opacity="0.5">
                        <path
                          d="M1.66663 10.8248V12.4998C1.66663 16.6665 3.33329 18.3332 7.49996 18.3332H12.5C16.6666 18.3332 18.3333 16.6665 18.3333 12.4998V7.49984C18.3333 3.33317 16.6666 1.6665 12.5 1.6665H7.49996C3.33329 1.6665 1.66663 3.33317 1.66663 7.49984"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.05835 8.8667L10 11.8L12.9417 8.8667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </g>
                    </svg>
                  </div>
                </div>
                <div className="iwmb2b-1">
                  <div className="iwmb2b-box">
                    <span className="iwmb2b-text-1">120 days</span>
                    <span className="iwmb2b-text-2">
                      (created on 10/12/2021)
                    </span>
                  </div>
                </div>
                <div className="iwmb2b-2">
                  <div className="iwmb2b-box">
                    <span className="iwmb2b-text-1">120 days</span>
                    <span className="iwmb2b-text-2">
                      (created on 10/12/2021)
                    </span>
                  </div>
                </div>
                <div className="iwmb2b-3">
                  <div className="iwmb2b-box">
                    <span className="iwmb2b-text-1">120 days</span>
                    <span className="iwmb2b-text-2">
                      (created on 10/12/2021)
                    </span>
                  </div>
                </div>
                <div className="iwmb2b-4">
                  <div className="iwmb2b-box">
                    <span className="iwmb2b-text-1">120 days</span>
                    <span className="iwmb2b-text-2">
                      (created on 10/12/2021)
                    </span>
                  </div>
                </div>
                <div className="iwmb2b-5">
                  <div className="iwmb2b-box">
                    <span className="iwmb2b-text-1">120 days</span>
                    <span className="iwmb2b-text-2">
                      (created on 10/12/2021)
                    </span>
                  </div>
                </div>
              </div>
              <div className="iwmb-3">
                <div className="iwmb3-head">
                  <span className="iwmb3h-text">Total Enrolled</span>
                  <div className="iwmb3h-vct">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 20 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g opacity="0.5">
                        <path
                          d="M1.66663 10.8248V12.4998C1.66663 16.6665 3.33329 18.3332 7.49996 18.3332H12.5C16.6666 18.3332 18.3333 16.6665 18.3333 12.4998V7.49984C18.3333 3.33317 16.6666 1.6665 12.5 1.6665H7.49996C3.33329 1.6665 1.66663 3.33317 1.66663 7.49984"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.05835 8.8667L10 11.8L12.9417 8.8667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </g>
                    </svg>
                  </div>
                </div>
                <div className="iwmb3b-1">
                  <div className="iwmb3b-box">
                    <span className="iwmb3b-text">20,000</span>
                  </div>
                </div>
                <div className="iwmb3b-2">
                  <div className="iwmb3b-box">
                    <span className="iwmb3b-text">20,000</span>
                  </div>
                </div>
                <div className="iwmb3b-3">
                  <div className="iwmb3b-box">
                    <span className="iwmb3b-text">20,000</span>
                  </div>
                </div>
                <div className="iwmb3b-4">
                  <div className="iwmb3b-box">
                    <span className="iwmb3b-text">20,000</span>
                  </div>
                </div>
                <div className="iwmb3b-5">
                  <div className="iwmb3b-box">
                    <span className="iwmb3b-text">20,000</span>
                  </div>
                </div>
              </div>
              <div className="iwmb-4">
                <div className="iwmb4-head">
                  <span className="iwmb4h-text">Qualified Candidates</span>
                  <div className="iwmb4h-vct">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 20 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g opacity="0.5">
                        <path
                          d="M1.66663 10.8248V12.4998C1.66663 16.6665 3.33329 18.3332 7.49996 18.3332H12.5C16.6666 18.3332 18.3333 16.6665 18.3333 12.4998V7.49984C18.3333 3.33317 16.6666 1.6665 12.5 1.6665H7.49996C3.33329 1.6665 1.66663 3.33317 1.66663 7.49984"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.05835 8.8667L10 11.8L12.9417 8.8667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </g>
                    </svg>
                  </div>
                </div>
                <div className="iwmb4b-1">
                  <div className="iwmb4b-box">
                    <div className="iwmb4b-boxmini">
                      <span className="iwmb4b-bxmtext">120</span>
                    </div>
                    <div className="iwmb4b-boxmax">
                      <div className="a"></div>
                      <div className="b"></div>
                      <div className="c"></div>
                      <div className="d"></div>
                      <div className="e"></div>
                      <div className="f"></div>
                      <div className="g"></div>
                      <div className="h"></div>
                      <div className="i"></div>
                      <div className="j"></div>
                      <div className="k"></div>
                      <div className="l"></div>
                      <div className="m"></div>
                      <div className="n"></div>
                      <div className="o"></div>
                      <div className="p"></div>
                      <div className="q"></div>
                      <div className="r"></div>
                      <div className="s"></div>
                      <div className="t"></div>
                    </div>
                  </div>
                </div>
                <div className="iwmb4b-2">
                  <div className="iwmb4b-box">
                    <div className="iwmb4b-boxmini">
                      <span className="iwmb4b-bxmtext">120</span>
                    </div>
                    <div className="iwmb4b-boxmax">
                      <div className="a"></div>
                      <div className="b"></div>
                      <div className="c"></div>
                      <div className="d"></div>
                      <div className="e"></div>
                      <div className="f"></div>
                      <div className="g"></div>
                      <div className="h"></div>
                      <div className="i"></div>
                      <div className="j"></div>
                      <div className="k"></div>
                      <div className="l"></div>
                      <div className="m"></div>
                      <div className="n"></div>
                      <div className="o"></div>
                      <div className="p"></div>
                      <div className="q"></div>
                      <div className="r"></div>
                      <div className="s"></div>
                      <div className="t"></div>
                    </div>
                  </div>
                </div>
                <div className="iwmb4b-3">
                  <div className="iwmb4b-box">
                    <div className="iwmb4b-boxmini">
                      <span className="iwmb4b-bxmtext">120</span>
                    </div>
                    <div className="iwmb4b-boxmax">
                      <div className="a"></div>
                      <div className="b"></div>
                      <div className="c"></div>
                      <div className="d"></div>
                      <div className="e"></div>
                      <div className="f"></div>
                      <div className="g"></div>
                      <div className="h"></div>
                      <div className="i"></div>
                      <div className="j"></div>
                      <div className="k"></div>
                      <div className="l"></div>
                      <div className="m"></div>
                      <div className="n"></div>
                      <div className="o"></div>
                      <div className="p"></div>
                      <div className="q"></div>
                      <div className="r"></div>
                      <div className="s"></div>
                      <div className="t"></div>
                    </div>
                  </div>
                </div>
                <div className="iwmb4b-4">
                  <div className="iwmb4b-box">
                    <div className="iwmb4b-boxmini">
                      <span className="iwmb4b-bxmtext">120</span>
                    </div>
                    <div className="iwmb4b-boxmax">
                      <div className="a"></div>
                      <div className="b"></div>
                      <div className="c"></div>
                      <div className="d"></div>
                      <div className="e"></div>
                      <div className="f"></div>
                      <div className="g"></div>
                      <div className="h"></div>
                      <div className="i"></div>
                      <div className="j"></div>
                      <div className="k"></div>
                      <div className="l"></div>
                      <div className="m"></div>
                      <div className="n"></div>
                      <div className="o"></div>
                      <div className="p"></div>
                      <div className="q"></div>
                      <div className="r"></div>
                      <div className="s"></div>
                      <div className="t"></div>
                    </div>
                  </div>
                </div>
                <div className="iwmb4b-5">
                  <div className="iwmb4b-box">
                    <div className="iwmb4b-boxmini">
                      <span className="iwmb4b-bxmtext">120</span>
                    </div>
                    <div className="iwmb4b-boxmax">
                      <div className="a"></div>
                      <div className="b"></div>
                      <div className="c"></div>
                      <div className="d"></div>
                      <div className="e"></div>
                      <div className="f"></div>
                      <div className="g"></div>
                      <div className="h"></div>
                      <div className="i"></div>
                      <div className="j"></div>
                      <div className="k"></div>
                      <div className="l"></div>
                      <div className="m"></div>
                      <div className="n"></div>
                      <div className="o"></div>
                      <div className="p"></div>
                      <div className="q"></div>
                      <div className="r"></div>
                      <div className="s"></div>
                      <div className="t"></div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="iwmb-5">
                <div className="iwmb5-head">
                  <span className="iwmb5h-text"></span>
                </div>
                <div className="iwmb5-body-1">
                  <div className="iwmb5b-box">
                    <div className="vct1">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M2.33337 25.6665H25.6667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-miterlimit="10"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M11.375 4.66683V25.6668H16.625V4.66683C16.625 3.3835 16.1 2.3335 14.525 2.3335H13.475C11.9 2.3335 11.375 3.3835 11.375 4.66683Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M8.16667 11.6668C8.16667 10.3835 7.7 9.3335 6.3 9.3335H5.36667C3.96667 9.3335 3.5 10.3835 3.5 11.6668V25.6668H8.16667V16.3218"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.8334 17.4998V25.6665H24.5V17.4998C24.5 16.2165 24.0334 15.1665 22.6334 15.1665H21.7C20.3 15.1665 19.8334 16.2165 19.8334 17.4998Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct2">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M14.7933 4.62016C15.3533 5.4485 15.6799 6.44016 15.6799 7.5135C15.6683 10.3135 13.4633 12.5885 10.6866 12.6818C10.5699 12.6702 10.4299 12.6702 10.3016 12.6818C7.52495 12.5885 5.31995 10.3135 5.31995 7.5135C5.31995 4.65516 7.62995 2.3335 10.4999 2.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.145 4.6665C21.4083 4.6665 23.2283 6.49817 23.2283 8.74984C23.2283 10.9548 21.4783 12.7515 19.2966 12.8332C19.2033 12.8215 19.0983 12.8215 18.9933 12.8332"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M4.85334 16.9865C2.03001 18.8765 2.03001 21.9565 4.85334 23.8348C8.06167 25.9815 13.3233 25.9815 16.5317 23.8348C19.355 21.9448 19.355 18.8648 16.5317 16.9865C13.335 14.8515 8.07334 14.8515 4.85334 16.9865Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M21.3967 23.3335C22.2367 23.1585 23.0301 22.8202 23.6834 22.3185C25.5034 20.9535 25.5034 18.7018 23.6834 17.3368C23.0417 16.8468 22.2601 16.5202 21.4317 16.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct3">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M11.48 8.3535C11.48 9.74183 12.6116 10.8735 14 10.8735C15.3883 10.8735 16.52 9.74183 16.52 8.3535C16.52 6.96516 15.3883 5.8335 14 5.8335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.92161 22.1665C9.30994 22.1665 10.4416 21.0348 10.4416 19.6465C10.4416 18.2581 9.30994 17.1265 7.92161 17.1265C6.53328 17.1265 5.40161 18.2581 5.40161 19.6465C5.40161 21.0348 6.52161 22.1665 7.92161 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M20.0783 22.1665C21.4667 22.1665 22.5984 21.0348 22.5984 19.6465C22.5984 18.2581 21.4667 17.1265 20.0783 17.1265C18.69 17.1265 17.5583 18.2581 17.5583 19.6465C17.5583 21.0348 18.69 22.1665 20.0783 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                  </div>
                </div>
                <div className="iwmb5-body-2">
                  <div className="iwmb5b-box">
                    <div className="vct1">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M2.33337 25.6665H25.6667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-miterlimit="10"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M11.375 4.66683V25.6668H16.625V4.66683C16.625 3.3835 16.1 2.3335 14.525 2.3335H13.475C11.9 2.3335 11.375 3.3835 11.375 4.66683Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M8.16667 11.6668C8.16667 10.3835 7.7 9.3335 6.3 9.3335H5.36667C3.96667 9.3335 3.5 10.3835 3.5 11.6668V25.6668H8.16667V16.3218"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.8334 17.4998V25.6665H24.5V17.4998C24.5 16.2165 24.0334 15.1665 22.6334 15.1665H21.7C20.3 15.1665 19.8334 16.2165 19.8334 17.4998Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct2">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M14.7933 4.62016C15.3533 5.4485 15.6799 6.44016 15.6799 7.5135C15.6683 10.3135 13.4633 12.5885 10.6866 12.6818C10.5699 12.6702 10.4299 12.6702 10.3016 12.6818C7.52495 12.5885 5.31995 10.3135 5.31995 7.5135C5.31995 4.65516 7.62995 2.3335 10.4999 2.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.145 4.6665C21.4083 4.6665 23.2283 6.49817 23.2283 8.74984C23.2283 10.9548 21.4783 12.7515 19.2966 12.8332C19.2033 12.8215 19.0983 12.8215 18.9933 12.8332"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M4.85334 16.9865C2.03001 18.8765 2.03001 21.9565 4.85334 23.8348C8.06167 25.9815 13.3233 25.9815 16.5317 23.8348C19.355 21.9448 19.355 18.8648 16.5317 16.9865C13.335 14.8515 8.07334 14.8515 4.85334 16.9865Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M21.3967 23.3335C22.2367 23.1585 23.0301 22.8202 23.6834 22.3185C25.5034 20.9535 25.5034 18.7018 23.6834 17.3368C23.0417 16.8468 22.2601 16.5202 21.4317 16.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct3">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M11.48 8.3535C11.48 9.74183 12.6116 10.8735 14 10.8735C15.3883 10.8735 16.52 9.74183 16.52 8.3535C16.52 6.96516 15.3883 5.8335 14 5.8335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.92161 22.1665C9.30994 22.1665 10.4416 21.0348 10.4416 19.6465C10.4416 18.2581 9.30994 17.1265 7.92161 17.1265C6.53328 17.1265 5.40161 18.2581 5.40161 19.6465C5.40161 21.0348 6.52161 22.1665 7.92161 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M20.0783 22.1665C21.4667 22.1665 22.5984 21.0348 22.5984 19.6465C22.5984 18.2581 21.4667 17.1265 20.0783 17.1265C18.69 17.1265 17.5583 18.2581 17.5583 19.6465C17.5583 21.0348 18.69 22.1665 20.0783 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                  </div>
                </div>
                <div className="iwmb5-body-3">
                  <div className="iwmb5b-box">
                    <div className="vct1">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M2.33337 25.6665H25.6667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-miterlimit="10"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M11.375 4.66683V25.6668H16.625V4.66683C16.625 3.3835 16.1 2.3335 14.525 2.3335H13.475C11.9 2.3335 11.375 3.3835 11.375 4.66683Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M8.16667 11.6668C8.16667 10.3835 7.7 9.3335 6.3 9.3335H5.36667C3.96667 9.3335 3.5 10.3835 3.5 11.6668V25.6668H8.16667V16.3218"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.8334 17.4998V25.6665H24.5V17.4998C24.5 16.2165 24.0334 15.1665 22.6334 15.1665H21.7C20.3 15.1665 19.8334 16.2165 19.8334 17.4998Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct2">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M14.7933 4.62016C15.3533 5.4485 15.6799 6.44016 15.6799 7.5135C15.6683 10.3135 13.4633 12.5885 10.6866 12.6818C10.5699 12.6702 10.4299 12.6702 10.3016 12.6818C7.52495 12.5885 5.31995 10.3135 5.31995 7.5135C5.31995 4.65516 7.62995 2.3335 10.4999 2.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.145 4.6665C21.4083 4.6665 23.2283 6.49817 23.2283 8.74984C23.2283 10.9548 21.4783 12.7515 19.2966 12.8332C19.2033 12.8215 19.0983 12.8215 18.9933 12.8332"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M4.85334 16.9865C2.03001 18.8765 2.03001 21.9565 4.85334 23.8348C8.06167 25.9815 13.3233 25.9815 16.5317 23.8348C19.355 21.9448 19.355 18.8648 16.5317 16.9865C13.335 14.8515 8.07334 14.8515 4.85334 16.9865Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M21.3967 23.3335C22.2367 23.1585 23.0301 22.8202 23.6834 22.3185C25.5034 20.9535 25.5034 18.7018 23.6834 17.3368C23.0417 16.8468 22.2601 16.5202 21.4317 16.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct3">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M11.48 8.3535C11.48 9.74183 12.6116 10.8735 14 10.8735C15.3883 10.8735 16.52 9.74183 16.52 8.3535C16.52 6.96516 15.3883 5.8335 14 5.8335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.92161 22.1665C9.30994 22.1665 10.4416 21.0348 10.4416 19.6465C10.4416 18.2581 9.30994 17.1265 7.92161 17.1265C6.53328 17.1265 5.40161 18.2581 5.40161 19.6465C5.40161 21.0348 6.52161 22.1665 7.92161 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M20.0783 22.1665C21.4667 22.1665 22.5984 21.0348 22.5984 19.6465C22.5984 18.2581 21.4667 17.1265 20.0783 17.1265C18.69 17.1265 17.5583 18.2581 17.5583 19.6465C17.5583 21.0348 18.69 22.1665 20.0783 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                  </div>
                </div>
                <div className="iwmb5-body-4">
                  <div className="iwmb5b-box">
                    <div className="vct1">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M2.33337 25.6665H25.6667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-miterlimit="10"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M11.375 4.66683V25.6668H16.625V4.66683C16.625 3.3835 16.1 2.3335 14.525 2.3335H13.475C11.9 2.3335 11.375 3.3835 11.375 4.66683Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M8.16667 11.6668C8.16667 10.3835 7.7 9.3335 6.3 9.3335H5.36667C3.96667 9.3335 3.5 10.3835 3.5 11.6668V25.6668H8.16667V16.3218"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.8334 17.4998V25.6665H24.5V17.4998C24.5 16.2165 24.0334 15.1665 22.6334 15.1665H21.7C20.3 15.1665 19.8334 16.2165 19.8334 17.4998Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct2">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M14.7933 4.62016C15.3533 5.4485 15.6799 6.44016 15.6799 7.5135C15.6683 10.3135 13.4633 12.5885 10.6866 12.6818C10.5699 12.6702 10.4299 12.6702 10.3016 12.6818C7.52495 12.5885 5.31995 10.3135 5.31995 7.5135C5.31995 4.65516 7.62995 2.3335 10.4999 2.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.145 4.6665C21.4083 4.6665 23.2283 6.49817 23.2283 8.74984C23.2283 10.9548 21.4783 12.7515 19.2966 12.8332C19.2033 12.8215 19.0983 12.8215 18.9933 12.8332"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M4.85334 16.9865C2.03001 18.8765 2.03001 21.9565 4.85334 23.8348C8.06167 25.9815 13.3233 25.9815 16.5317 23.8348C19.355 21.9448 19.355 18.8648 16.5317 16.9865C13.335 14.8515 8.07334 14.8515 4.85334 16.9865Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M21.3967 23.3335C22.2367 23.1585 23.0301 22.8202 23.6834 22.3185C25.5034 20.9535 25.5034 18.7018 23.6834 17.3368C23.0417 16.8468 22.2601 16.5202 21.4317 16.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct3">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M11.48 8.3535C11.48 9.74183 12.6116 10.8735 14 10.8735C15.3883 10.8735 16.52 9.74183 16.52 8.3535C16.52 6.96516 15.3883 5.8335 14 5.8335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.92161 22.1665C9.30994 22.1665 10.4416 21.0348 10.4416 19.6465C10.4416 18.2581 9.30994 17.1265 7.92161 17.1265C6.53328 17.1265 5.40161 18.2581 5.40161 19.6465C5.40161 21.0348 6.52161 22.1665 7.92161 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M20.0783 22.1665C21.4667 22.1665 22.5984 21.0348 22.5984 19.6465C22.5984 18.2581 21.4667 17.1265 20.0783 17.1265C18.69 17.1265 17.5583 18.2581 17.5583 19.6465C17.5583 21.0348 18.69 22.1665 20.0783 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                  </div>
                </div>
                <div className="iwmb5-body-5">
                  <div className="iwmb5b-box">
                    <div className="vct1">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M2.33337 25.6665H25.6667"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-miterlimit="10"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M11.375 4.66683V25.6668H16.625V4.66683C16.625 3.3835 16.1 2.3335 14.525 2.3335H13.475C11.9 2.3335 11.375 3.3835 11.375 4.66683Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M8.16667 11.6668C8.16667 10.3835 7.7 9.3335 6.3 9.3335H5.36667C3.96667 9.3335 3.5 10.3835 3.5 11.6668V25.6668H8.16667V16.3218"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.8334 17.4998V25.6665H24.5V17.4998C24.5 16.2165 24.0334 15.1665 22.6334 15.1665H21.7C20.3 15.1665 19.8334 16.2165 19.8334 17.4998Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct2">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M14.7933 4.62016C15.3533 5.4485 15.6799 6.44016 15.6799 7.5135C15.6683 10.3135 13.4633 12.5885 10.6866 12.6818C10.5699 12.6702 10.4299 12.6702 10.3016 12.6818C7.52495 12.5885 5.31995 10.3135 5.31995 7.5135C5.31995 4.65516 7.62995 2.3335 10.4999 2.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M19.145 4.6665C21.4083 4.6665 23.2283 6.49817 23.2283 8.74984C23.2283 10.9548 21.4783 12.7515 19.2966 12.8332C19.2033 12.8215 19.0983 12.8215 18.9933 12.8332"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M4.85334 16.9865C2.03001 18.8765 2.03001 21.9565 4.85334 23.8348C8.06167 25.9815 13.3233 25.9815 16.5317 23.8348C19.355 21.9448 19.355 18.8648 16.5317 16.9865C13.335 14.8515 8.07334 14.8515 4.85334 16.9865Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M21.3967 23.3335C22.2367 23.1585 23.0301 22.8202 23.6834 22.3185C25.5034 20.9535 25.5034 18.7018 23.6834 17.3368C23.0417 16.8468 22.2601 16.5202 21.4317 16.3335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                    <div className="vct3">
                      <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M11.48 8.3535C11.48 9.74183 12.6116 10.8735 14 10.8735C15.3883 10.8735 16.52 9.74183 16.52 8.3535C16.52 6.96516 15.3883 5.8335 14 5.8335"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M7.92161 22.1665C9.30994 22.1665 10.4416 21.0348 10.4416 19.6465C10.4416 18.2581 9.30994 17.1265 7.92161 17.1265C6.53328 17.1265 5.40161 18.2581 5.40161 19.6465C5.40161 21.0348 6.52161 22.1665 7.92161 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                        <path
                          d="M20.0783 22.1665C21.4667 22.1665 22.5984 21.0348 22.5984 19.6465C22.5984 18.2581 21.4667 17.1265 20.0783 17.1265C18.69 17.1265 17.5583 18.2581 17.5583 19.6465C17.5583 21.0348 18.69 22.1665 20.0783 22.1665Z"
                          stroke="#793EF5"
                          stroke-width="1.5"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}

export default Internships;
